...
export abstract class IOSProtocol extends ProtocolAdapter {
    ...
    constructor(target: Target) {
        ...
        this._target.on("tools::DOM.getDocument", () =>
            this.onDomGetDocument()
        );

        this._target.addMessageFilter("tools::CSS.setStyleTexts", (msg) =>
            this.onSetStyleTexts(msg)
        );
        this._target.addMessageFilter(
            "tools::CSS.getMatchedStylesForNode",
            (msg) => this.onGetMatchedStylesForNode(msg)
        );
        this._target.addMessageFilter("tools::CSS.getBackgroundColors", (msg) =>
            this.onGetBackgroundColors(msg)
        );
        this._target.addMessageFilter("tools::CSS.addRule", (msg) =>
            this.onAddRule(msg)
        );
        this._target.addMessageFilter(
            "tools::CSS.getPlatformFontsForNode",
            (msg) => this.onGetPlatformFontsForNode(msg)
        );
        this._target.addMessageFilter(
            "target::CSS.getMatchedStylesForNode",
            (msg) => this.onGetMatchedStylesForNodeResult(msg)
        );

        this._target.addMessageFilter("tools::Page.startScreencast", (msg) =>
            this.onStartScreencast(msg)
        );
        this._target.addMessageFilter("tools::Page.stopScreencast", (msg) =>
            this.onStopScreencast(msg)
        );
        this._target.addMessageFilter("tools::Page.screencastFrameAck", (msg) =>
            this.onScreencastFrameAck(msg)
        );
        this._target.addMessageFilter(
            "tools::Page.getNavigationHistory",
            (msg) => this.onGetNavigationHistory(msg)
        );
        this._target.addMessageFilter(
            "tools::Page.setOverlayMessage",
            (msg) => {
        ...
        this._target.addMessageFilter("tools::DOM.enable", (msg) =>
            this.onDomEnable(msg)
        );
        this._target.addMessageFilter("tools::DOM.setInspectMode", (msg) =>
            this.onSetInspectMode(msg)
        );
        ...
        this._target.addMessageFilter(
            "tools::DOM.pushNodesByBackendIdsToFrontend",
            (msg) => this.onPushNodesByBackendIdsToFrontend(msg)
        );
        this._target.addMessageFilter("tools::DOM.getBoxModel", (msg) =>
            this.onGetBoxModel(msg)
        );
        this._target.addMessageFilter("tools::DOM.getNodeForLocation", (msg) =>
            this.onGetNodeForLocation(msg)
        );

        this._target.addMessageFilter(
            "tools::DOMDebugger.getEventListeners",
            (msg) => this.DOMDebuggerOnGetEventListeners(msg)
        );

        this._target.addMessageFilter(
            "tools::Debugger.canSetScriptSource",
            (msg) => this.onCanSetScriptSource(msg)
        );
        this._target.addMessageFilter(
            "tools::Debugger.setBlackboxPatterns",
            (msg) => this.onSetBlackboxPatterns(msg)
        );
        this._target.addMessageFilter(
            "tools::Debugger.setAsyncCallStackDepth",
            (msg) => this.onSetAsyncCallStackDepth(msg)
        );
        this._target.addMessageFilter("tools::Debugger.enable", (msg) =>
            this.onDebuggerEnable(msg)
        );
        this._target.addMessageFilter("target::Debugger.scriptParsed", (msg) =>
            this.onScriptParsed(msg)
        );

        this._target.addMessageFilter("tools::Emulation.canEmulate", (msg) =>
            this.onCanEmulate(msg)
        );
        this._target.addMessageFilter(
            "tools::Emulation.setTouchEmulationEnabled",
            (msg) => {
        ...
        this._target.addMessageFilter(
            "tools::Emulation.setScriptExecutionDisabled",
            (msg) => {
        ...
        this._target.addMessageFilter(
            "tools::Emulation.setEmulatedMedia",
            (msg) => {
        ...
        this._target.addMessageFilter(
            "tools::Rendering.setShowPaintRects",
            (msg) => {
        ...
        this._target.addMessageFilter(
            "tools::Input.emulateTouchFromMouseEvent",
            (msg) => this.onEmulateTouchFromMouseEvent(msg)
        );
        ...
        this._target.addMessageFilter("target::Console.messageAdded", (msg) =>
            this.onConsoleMessageAdded(msg)
        );
        ...
        this._target.addMessageFilter(
            "tools::Network.setMonitoringXHREnabled",
            (msg) => {
        ...
        this._target.addMessageFilter(
            "tools::Network.canEmulateNetworkConditions",
            (msg) => this.onCanEmulateNetworkConditions(msg)
        );

        this._target.addMessageFilter("tools::Runtime.compileScript", (msg) =>
            this.onRuntimeOnCompileScript(msg)
        );
        this._target.addMessageFilter(
            "target::Runtime.executionContextCreated",
            (msg) => this.onExecutionContextCreated(msg)
        );
        this._target.addMessageFilter("target::Runtime.evaluate", (msg) =>
            this.onEvaluate(msg)
        );
        this._target.addMessageFilter("target::Runtime.getProperties", (msg) =>
            this.onRuntimeGetProperties(msg)
        );

        this._target.addMessageFilter("target::Inspector.inspect", (msg) =>
            this.onInspect(msg)
        );
    }
            ...
            const setStyleText = this._target
                ...
                .then((result) => {
                    if (!result.styleSheet || !result.styleSheet.rules) {
                        Logger.error(
                            "iOS returned a value we were not expecting for getStyleSheet"
                        );
                ...
                if (result.inherited[i].matchedCSSRules) {
                    for (let j in result.inherited[i].matchedCSSRules) {
                        if (result.inherited[i].matchedCSSRules[j].rule) {
                            this.mapRule(
                                result.inherited[i].matchedCSSRules[j].rule
                            );
                        }
                    }
                }
    ...
    private DOMDebuggerOnGetEventListeners(msg: any): Promise<any> {
        ...
        this._target
            ...
            .then((result) => {
                ...
                return this._target.callTarget(
                    "DOM.getEventListenersForNode",
                    getEventListenersForNodeParams
                );
            })
            .then((result) => {
    ...
    private onStartScreencast(msg: any): Promise<any> {
        ...
        this._screencastSession = new ScreencastSession(
            this._target,
            format,
            quality,
            maxWidth,
    ...
    private onEmulateTouchFromMouseEvent(msg: any): Promise<any> {
        ...
        switch (msg.params.type) {
            ...
            default:
                Logger.error(
                    `Unknown emulate mouse event name '${msg.params.type}'`
                );
                break;
        }
    ...
    private static extractDisabledStyles(
        ...
        for (
            let index = 0, length = styleText.length;
            index < length;
            index++
        ) {
            if (
                styleText.substr(index, IOSProtocol.BEGIN_COMMENT.length) ===
                IOSProtocol.BEGIN_COMMENT
            ) {
                startIndices.push(index);
                index = index + IOSProtocol.BEGIN_COMMENT.length - 1;
            } else if (
                styleText.substr(index, IOSProtocol.END_COMMENT.length) ===
